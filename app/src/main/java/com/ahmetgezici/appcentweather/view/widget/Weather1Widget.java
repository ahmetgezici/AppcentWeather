...
public class Weather1Widget extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        String woeid = Widget1ConfigActivity.loadTitlePref(context, appWidgetId);

        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.weather1_widget);
            ...
            public void onResponse(@NotNull Call<WeatherResponse> call, @NotNull Response<WeatherResponse> response) {
                if (response.code() == 200) {
                    ...
                    if (weatherResponse != null) {
                        ...
                        views.setViewVisibility(R.id.rootLayout, View.VISIBLE);

                        Intent intent = new Intent(context, MainActivity.class);
                        intent.putExtra("woeid", woeid);
                        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        views.setOnClickPendingIntent(R.id.rootLayout, PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT));
                    }
                }
            }
